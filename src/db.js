...
const stmts = {
  ...
  insertCronJob: db.prepare(`INSERT INTO cron_jobs(id, name, schedule, target_type, target_url, http_method, http_headers, http_body, container_id, container_cmd, retries, timeout_ms, timezone, failure_webhook, is_active) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),
  updateCronJob: db.prepare(`UPDATE cron_jobs SET name =?, schedule =?, target_type =?, target_url =?, http_method =?, http_headers =?, http_body =?, container_id =?, container_cmd =?, retries =?, timeout_ms =?, timezone =?, failure_webhook =?, is_active =?, updated_at = datetime('now') WHERE id =? `),
